...
# ADASTRA data & MPRA data
```{r}
# Read ADASTRA data
ADASTRA_data <- read.delim(paste(paste(Working_dir, "Input_Files/ADASTRA_data", sep = "/"), "AD_ASTRA_EGR1_HUMAN.tsv", sep = "/"), header = TRUE, sep = "\t")

# Merge
ADASTRA_data_DF <- merge(Combined_DF, ADASTRA_data, by.x = "SNP", by.y = "ID")

# plot
plot(ADASTRA_data_DF$Diff_log2FC_alt_minus_ref, 
     ADASTRA_data_DF$es_mean_alt - ADASTRA_data_DF$es_mean_ref, 
     pch = 19, cex = 2)
abline(h = 0, col = "black")
abline(v = 0, col = "black")
...
# Read ADASTRA data, obtain DNA sequences, and tile PADIT-seq data
```{r}
# Read ADASTRA data
ADASTRA_data <- read.delim(paste(paste(Working_dir, "Input_Files/ADASTRA_data", sep = "/"), "AD_ASTRA_EGR1_HUMAN.tsv", sep = "/"), header = TRUE, sep = "\t")
...
library(BSgenome.Hsapiens.UCSC.hg38)  # Make sure you have this installed
library(Biostrings)

# Function to get flanking sequences
get_flanking_sequences <- function(snp_data, flank_size = 20) {
  # Create GRanges object
  gr <- GRanges(
    seqnames = snp_data$X.chr,
    ranges = IRanges(
      start = snp_data$pos - flank_size,
      end = snp_data$pos + flank_size
    ),
    strand = "+",
  ...
  # Calculate position in extracted sequence where SNP occurs
  snp_pos <- flank_size + 1
  ...
  results <- data.frame(
    ...
    ref = snp_data$ref,
    alt = snp_data$alt,
    ref_seq = sapply(seq_along(seqs), function(i) {
      seq <- as.character(seqs[i])
      paste0(
        substr(seq, 1, snp_pos-1),
        snp_data$ref[i],
        substr(seq, snp_pos+1, nchar(seq))
      )
    }),
    alt_seq = sapply(seq_along(seqs), function(i) {
      seq <- as.character(seqs[i])
      paste0(
        substr(seq, 1, snp_pos-1),
        snp_data$alt[i],
...
# Use the function
flanking_sequences <- get_flanking_sequences(ADASTRA_data, flank_size = 20)

# 
colnames(flanking_sequences) <- c("rsid", "chr", "pos", "ref", "alt", "DNA_REF", "DNA_ALT")

TEMP <- merge(flanking_sequences, ADASTRA_data, by.x = "rsid", by.y = "ID")
EGR1_SNP_SELEX <- TEMP; rm(TEMP)
...
# Plot ADASTRA data and PADIT-seq data
```{r}
# subset
EGR1_FDR_0.05 <- read.delim(paste(paste(Working_dir, "Input_Files/ADASTRA_data", sep = "/"), "AD_ASTRA_search_results_EGR1_FDR_0.05.tsv", sep = "/"), header = TRUE, sep = "\t")
EGR1_SNP_SELEX_FDR_05 <- EGR1_SNP_SELEX[which(EGR1_SNP_SELEX$rsid %in% EGR1_FDR_0.05$rsID),]

# table
table(EGR1_SNP_SELEX_FDR_05$PADIT_prediction, 
      EGR1_SNP_SELEX_FDR_05$motif_conc)

# subset
EGR1_SNP_SELEX_FDR_05 <- EGR1_SNP_SELEX_FDR_05[which(EGR1_SNP_SELEX_FDR_05$PADIT_prediction == 0),]
EGR1_SNP_SELEX_FDR_05 <- EGR1_SNP_SELEX_FDR_05[complete.cases(EGR1_SNP_SELEX_FDR_05),]

# Assign discordant 
EGR1_SNP_SELEX_FDR_05$PADIT_Concordance <- "red"
EGR1_SNP_SELEX_FDR_05$PADIT_Concordance[which(EGR1_SNP_SELEX_FDR_05$Diff_log2FC > 0 & (EGR1_SNP_SELEX_FDR_05$es_mean_ref - EGR1_SNP_SELEX_FDR_05$es_mean_alt) < 0)] <- "black"
EGR1_SNP_SELEX_FDR_05$PADIT_Concordance[which(EGR1_SNP_SELEX_FDR_05$Diff_log2FC < 0 & (EGR1_SNP_SELEX_FDR_05$es_mean_ref - EGR1_SNP_SELEX_FDR_05$es_mean_alt) > 0)] <- "black"

# plot
EGR1_SNP_SELEX_FDR_05$Colour <- "black"
EGR1_SNP_SELEX_FDR_05$Colour[which(EGR1_SNP_SELEX_FDR_05$motif_conc %in% c("Discordant", "Weak Discordant"))] <- "red"
plot(EGR1_SNP_SELEX_FDR_05$Diff_log2FC, 
     EGR1_SNP_SELEX_FDR_05$es_mean_ref - EGR1_SNP_SELEX_FDR_05$es_mean_alt, 
     pch = 19, cex = 1.5, 
     xlab = "PADIT-seq (REF - ALT)", 
     ylab = "AD-ASTRA (REF - ALT)", 
     col = EGR1_SNP_SELEX_FDR_05$PADIT_Concordance)
abline(h = 0, col = "black")
abline(v = 0, col = "black")

# correlation
cor.test(EGR1_SNP_SELEX_FDR_05$Diff_log2FC, 
         EGR1_SNP_SELEX_FDR_05$es_mean_ref - EGR1_SNP_SELEX_FDR_05$es_mean_alt)
...
#################### 
snps.mb <- snps.from.rsid(rsid = EGR1_FDR_0.05$rsID,
...
Unique_SNP_ids_motifBreakR <- unique(results_df$SNP_id)
results_df_formatted <- data.frame()
for(i in 1:length(Unique_SNP_ids_motifBreakR))
{
  Current_SNP <- results_df[which(results_df$SNP_id %in% Unique_SNP_ids_motifBreakR[i]), ]
  if(nrow(Current_SNP) == 1)
  {
    results_df_formatted <- rbind(results_df_formatted, 
                                           Current_SNP)
    next
  }
  if(nrow(Current_SNP) > 1)
...
EGR1_FDR_0.05 <- read.delim(paste(paste(Working_dir, "Input_Files/ADASTRA_data", sep = "/"), "AD_ASTRA_search_results_EGR1_FDR_0.05.tsv", sep = "/"), header = TRUE, sep = "\t")
EGR1_SNP_SELEX_FDR_05 <- EGR1_SNP_SELEX[which(EGR1_SNP_SELEX$rsid %in% EGR1_FDR_0.05$rsID),]
EGR1_SNP_SELEX_FDR_05 <- EGR1_SNP_SELEX_FDR_05[which(EGR1_SNP_SELEX_FDR_05$PADIT_prediction == 1),]
EGR1_SNP_SELEX_FDR_05_MotifBreakR <- merge(EGR1_SNP_SELEX_FDR_05, results_df_formatted, 
                                           by.x = "rsid", by.y = "SNP_id", 
                                           all.x = TRUE, all.y = TRUE)
EGR1_SNP_SELEX_FDR_05_MotifBreakR$alleleEffectSize[is.na(EGR1_SNP_SELEX_FDR_05_MotifBreakR$alleleEffectSize)] <- 0

# plot
plot(-1*EGR1_SNP_SELEX_FDR_05_MotifBreakR$alleleEffectSize, 
     EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_ref - EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_alt, 
     pch = 19, cex = 1, 
     xlab = "PADIT-seq (REF - ALT)", 
     ylab = "AD-ASTRA (REF - ALT)")
abline(h = 0, col = "black")
...
# 
cor.test(-1*EGR1_SNP_SELEX_FDR_05_MotifBreakR$alleleEffectSize, 
         EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_ref - EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_alt)
...
EGR1_FDR_0.05 <- read.delim(paste(paste(Working_dir, "Input_Files/ADASTRA_data", sep = "/"), "AD_ASTRA_search_results_EGR1_FDR_0.05.tsv", sep = "/"), header = TRUE, sep = "\t")
EGR1_SNP_SELEX_FDR_05 <- EGR1_SNP_SELEX[which(EGR1_SNP_SELEX$rsid %in% EGR1_FDR_0.05$rsID),]
EGR1_SNP_SELEX_FDR_05_MotifBreakR <- merge(EGR1_SNP_SELEX_FDR_05, results_df_formatted, by.x = "rsid", by.y = "SNP_id")

# Assign discordant 
EGR1_SNP_SELEX_FDR_05_MotifBreakR$PADIT_Concordance <- "red"
EGR1_SNP_SELEX_FDR_05_MotifBreakR$PADIT_Concordance[which(EGR1_SNP_SELEX_FDR_05_MotifBreakR$alleleEffectSize < 0 & (EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_ref - EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_alt) < 0)] <- "black"
EGR1_SNP_SELEX_FDR_05_MotifBreakR$PADIT_Concordance[which(EGR1_SNP_SELEX_FDR_05_MotifBreakR$alleleEffectSize > 0 & (EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_ref - EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_alt) > 0)] <- "black"
...
plot(-1*EGR1_SNP_SELEX_FDR_05_MotifBreakR$alleleEffectSize, 
     EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_ref - EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_alt, 
     pch = 19, cex = 1.5, 
     xlab = "MOTIFBREAKR (REF - ALT)", 
     ylab = "AD-ASTRA (REF - ALT)", 
     col = EGR1_SNP_SELEX_FDR_05_MotifBreakR$PADIT_Concordance)
abline(h = 0, col = "black")
abline(v = 0, col = "black")

# 
table(EGR1_SNP_SELEX_FDR_05_MotifBreakR$PADIT_Concordance)
cor.test(-1*EGR1_SNP_SELEX_FDR_05_MotifBreakR$alleleEffectSize, 
         EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_ref - EGR1_SNP_SELEX_FDR_05_MotifBreakR$es_mean_alt)
