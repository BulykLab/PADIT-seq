...
#
df1 <- TBX5_PBM_Escores_PADIT[,c("X8.mer_1_8", "Mean_log2FC", "MPRAactivity")]
colnames(df1) <- c("TFBS", "log2FoldChange", "MPRAactivity")
df2 <- TBX5_PBM_Escores_PADIT[,c("X8.mer.1_1_8", "Mean_log2FC", "MPRAactivity")]
colnames(df2) <- c("TFBS", "log2FoldChange", "MPRAactivity")
dds_results_TBX5_df <- rbind(df1, df2)

```



# Tile PADIT-seq TBX5 8-mers on HT-SELEX reads
```{r}
for(Select_cycle in c("input", "cycle1", "cycle2", "cycle3", "cycle4"))
{
  # Read TBX5 SELEX-seq DNA sequences
  if(Select_cycle == "input")
  {
    TBX5_CHIP_tmp <- unique(read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_TBX5", sep = "/"), "ZeroCycle_TAGAAC40NAAT_0_0.fastq.txt", sep = "/"), header = FALSE))
  }
  if(Select_cycle == "cycle1")
  {
    TBX5_CHIP_tmp <- unique(read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_TBX5", sep = "/"), "TBX5_TAGAAC40NAAT_AI_1.fastq.txt", sep = "/"), header = FALSE))
  }
  if(Select_cycle == "cycle2")
  {
    TBX5_CHIP_tmp <- unique(read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_TBX5", sep = "/"), "TBX5_TAGAAC40NAAT_AI_2.fastq.txt", sep = "/"), header = FALSE))
  }
  if(Select_cycle == "cycle3")
  {
    TBX5_CHIP_tmp <- unique(read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_TBX5", sep = "/"), "TBX5_TAGAAC40NAAT_AI_3.fastq.txt", sep = "/"), header = FALSE))
  }
  if(Select_cycle == "cycle4")
  {
    TBX5_CHIP_tmp <- unique(read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_TBX5", sep = "/"), "TBX5_TAGAAC40NAAT_AI_4.fastq.txt", sep = "/"), header = FALSE))
  }
  
  #
  TBX5_CHIP <- as.data.frame(TBX5_CHIP_tmp[sample(1:nrow(TBX5_CHIP_tmp), 20000), ])
  colnames(TBX5_CHIP) <- c("V1")
  
  # Obtain tiling
  numPeaks <- nrow(TBX5_CHIP)
  Heatmap_DF_log2FC <- matrix(0, nrow = numPeaks, ncol = 13)
  Heatmap_DF_MPRAactivity <- matrix(0, nrow = numPeaks, ncol = 13)
  for(i in 1:numPeaks)
  {
   CurrentPeak <- toupper(TBX5_CHIP$V1[i])
   Sequence_10mers_DF <- data.frame()
   for(j in 1:13)
   {
     Sequence_10mers_DF[j, 1] <- substr(CurrentPeak, j, j+7)
   }
   dds_results_TBX5_df_subset <- dds_results_TBX5_df[which(dds_results_TBX5_df$TFBS %in% Sequence_10mers_DF$V1),]
   dds_results_TBX5_df_subset <- dds_results_TBX5_df_subset[match(Sequence_10mers_DF$V1, dds_results_TBX5_df_subset$TFBS),]
   Heatmap_DF_log2FC[i,] <- dds_results_TBX5_df_subset$log2FoldChange
   Heatmap_DF_MPRAactivity[i,] <- dds_results_TBX5_df_subset$MPRAactivity
  }
  
  # Write
  if(Select_cycle == "input")
  {
    write.table(Heatmap_DF_log2FC, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_SELEX_input.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
    write.table(Heatmap_DF_MPRAactivity, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_SELEX_input.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
  }
  if(Select_cycle == "cycle1")
  {
    write.table(Heatmap_DF_log2FC, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_HT-SELEX_cycle1.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
    write.table(Heatmap_DF_MPRAactivity, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_HT-SELEX_cycle1.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
  }
  if(Select_cycle == "cycle2")
  {
    write.table(Heatmap_DF_log2FC, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_HT-SELEX_cycle2.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
    write.table(Heatmap_DF_MPRAactivity, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_HT-SELEX_cycle2.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
  }
  if(Select_cycle == "cycle3")
  {
    write.table(Heatmap_DF_log2FC, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_HT-SELEX_cycle3.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
    write.table(Heatmap_DF_MPRAactivity, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_HT-SELEX_cycle3.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
  }
  if(Select_cycle == "cycle4")
  {
    write.table(Heatmap_DF_log2FC, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_HT-SELEX_cycle4.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
    write.table(Heatmap_DF_MPRAactivity, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_HT-SELEX_cycle4.txt", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
  }
}
...
# Input
Heatmap_DF_log2FC_input <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_SELEX_input.txt", sep = "/"))
Heatmap_DF_MPRAactivity_input <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_SELEX_input.txt", sep = "/"))
...
# Cycle-1
Heatmap_DF_log2FC_cycle1 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_HT-SELEX_cycle1.txt", sep = "/"))
Heatmap_DF_MPRAactivity_cycle1 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_HT-SELEX_cycle1.txt", sep = "/"))
...
# Cycle-2
Heatmap_DF_log2FC_cycle2 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_HT-SELEX_cycle2.txt", sep = "/"))
Heatmap_DF_MPRAactivity_cycle2 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_HT-SELEX_cycle2.txt", sep = "/"))
...
# Cycle-3
Heatmap_DF_log2FC_cycle3 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_HT-SELEX_cycle3.txt", sep = "/"))
Heatmap_DF_MPRAactivity_cycle3 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_HT-SELEX_cycle3.txt", sep = "/"))
...
# Cycle-4
Heatmap_DF_log2FC_cycle4 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_log2FC_8bpstiling_TBX5_HT-SELEX_cycle4.txt", sep = "/"))
Heatmap_DF_MPRAactivity_cycle4 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "Heatmap_DF_MPRAactivity_8bpstiling_TBX5_HT-SELEX_cycle4.txt", sep = "/"))
...
TBX5_PBM_All60mers <- read.delim(paste(paste(Working_dir, "Input_Files/PBM_data_TBX5", sep = "/"), "Tbx5_alldata.txt", sep = "/"), header = TRUE, sep = "\t")
TBX5_PBM_All60mers <- TBX5_PBM_All60mers[complete.cases(TBX5_PBM_All60mers),]

######################################################### Tile probes
TBX5_PBM_All60mers$Longest_1s <- 0
TBX5_PBM_All60mers$SecondLongest_1s <- 0
TBX5_PBM_All60mers$ZeroesBetween <- -1
for(i in 1:nrow(TBX5_PBM_All60mers))
{
  Current_DNA_sequence <- TBX5_PBM_All60mers$Sequence[i]
  ...
  dds_results_TBX5_df_subset <- dds_results_TBX5_df[which(dds_results_TBX5_df$TFBS %in% Sequence_8mers_DF$V1),]
  dds_results_TBX5_df_subset <- dds_results_TBX5_df_subset[match(Sequence_8mers_DF$V1, dds_results_TBX5_df_subset$TFBS),]
  TBX5_PBM_All60mers$num_active[i] <- sum(dds_results_TBX5_df_subset$MPRAactivity)
  TBX5_PBM_All60mers$sum_log2FC[i] <- sum(2^dds_results_TBX5_df_subset$log2FoldChange)
  TBX5_PBM_All60mers$max_log2FC_active_only[i] <- max(2^dds_results_TBX5_df_subset$log2FoldChange)
  if(sum(dds_results_TBX5_df_subset$MPRAactivity) == 0)
  {
    TBX5_PBM_All60mers$sum_log2FC_active_only[i] <- 0
    TBX5_PBM_All60mers$max_log2FC_active_only[i] <- 0
    next
  }
  TBX5_PBM_All60mers$sum_log2FC_active_only[i] <- sum(2^dds_results_TBX5_df_subset$log2FoldChange[which(dds_results_TBX5_df_subset$MPRAactivity == 1)])
  
  # Longest and second longest 1's
  rle_1s <- rle(dds_results_TBX5_df_subset$MPRAactivity)
  longest_second_vec <- sort(rle_1s$lengths[which(rle_1s$values == 1)], decreasing = TRUE)
  if(length(longest_second_vec) > 0)
  {
    TBX5_PBM_All60mers$Longest_1s[i] <- longest_second_vec[1]
  }
  if(length(longest_second_vec) > 1)
  {
    TBX5_PBM_All60mers$SecondLongest_1s[i] <- longest_second_vec[2]
    ...
    TBX5_PBM_All60mers$ZeroesBetween[i] <- zeros_between
  }
}

######################################################### Fit linear models
# subset to make sure there aren't any other active 9-mers
TBX5_PBM_All60mers_subset <- TBX5_PBM_All60mers[which(TBX5_PBM_All60mers$num_active == TBX5_PBM_All60mers$Longest_1s),]
# TBX5_PBM_All60mers_subset <- TBX5_PBM_All60mers_subset[which(TBX5_PBM_All60mers_subset$num_active > 0), ]

# run lm
summary(lm(log10(TBX5_PBM_All60mers_subset$Alexa488Adjusted) ~ TBX5_PBM_All60mers_subset$sum_log2FC_active_only + TBX5_PBM_All60mers_subset$max_log2FC_active_only))
summary(lm(log10(TBX5_PBM_All60mers_subset$Alexa488Adjusted) ~ TBX5_PBM_All60mers_subset$num_active + TBX5_PBM_All60mers_subset$max_log2FC_active_only))

# relative importance
library(relaimpo)
model <- lm(log10(Alexa488Adjusted) ~ max_log2FC_active_only + num_active, data = TBX5_PBM_All60mers_subset)
model <- lm(log10(Alexa488Adjusted) ~ max_log2FC_active_only + sum_log2FC_active_only, data = TBX5_PBM_All60mers_subset)
...
TBX5_PBM_All60mers_subset <- TBX5_PBM_All60mers[which(TBX5_PBM_All60mers$num_active == TBX5_PBM_All60mers$Longest_1s),]
TBX5_PBM_All60mers_subset$num_active[which(TBX5_PBM_All60mers_subset$num_active > 6)] <- 6
table(TBX5_PBM_All60mers_subset$num_active)
table(TBX5_PBM_All60mers$num_active)

# Boxplot
boxplot(log10(TBX5_PBM_All60mers_subset$Alexa488Adjusted) ~ TBX5_PBM_All60mers_subset$num_active, 
        notch = TRUE, outline = FALSE)
pairwise.wilcox.test(TBX5_PBM_All60mers_subset$Alexa488Adjusted,
                     TBX5_PBM_All60mers_subset$num_active)

######################################################### Boxplot - 2
# subset
TBX5_PBM_All60mers_subset <- TBX5_PBM_All60mers[which(TBX5_PBM_All60mers$Longest_1s + TBX5_PBM_All60mers$SecondLongest_1s == 5), ]
TBX5_PBM_All60mers_subset$interaction_term <- interaction(TBX5_PBM_All60mers_subset$Longest_1s, 
                                                            TBX5_PBM_All60mers_subset$SecondLongest_1s, 
                                                            drop = TRUE, lex.order = TRUE)

# Boxplot
boxplot(log10(TBX5_PBM_All60mers_subset$Alexa488Adjusted) ~ droplevels(TBX5_PBM_All60mers_subset$interaction_term), 
        notch = TRUE, outline = FALSE)
table(TBX5_PBM_All60mers_subset$interaction_term)
pairwise.wilcox.test(TBX5_PBM_All60mers_subset$Alexa488Adjusted,
                     TBX5_PBM_All60mers_subset$interaction_term)

```

# Consecutive overlapping 8-mer E-scores versus 10-mer PADIT-seq log2foldchange
```{r}
# Read TBX5 10-mer PADIT-seq data
dds_results_TBX5_df <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "dds_results_TBX5_df.txt", sep = "/"), header = TRUE)
dds_results_TBX5_df$MPRAactivity <- (dds_results_TBX5_df$log2FoldChange > 0 & dds_results_TBX5_df$padj < Cutoff_Value)*1

# Read and format TBX5 PBM data
TBX5_PBM_Escores <- read.table(paste(paste(Working_dir, "Input_Files/PBM_data_TBX5", sep = "/"), "Tbx5_8mers_11111111.txt", sep = "/"), header = TRUE)
TEMP1 <- TBX5_PBM_Escores[,c("X8.mer", "E.score")]
TEMP2 <- TBX5_PBM_Escores[,c("X8.mer.1", "E.score")]
colnames(TEMP1) <- c("X8.mer", "E.score")
colnames(TEMP2) <- c("X8.mer", "E.score")
TEMP <- rbind(TEMP1, TEMP2)
TBX5_PBM_Escores <- TEMP

# 
dds_results_TBX5_df$mer8_1 <- substr(rownames(dds_results_TBX5_df), 1, 8)
dds_results_TBX5_df$mer8_2 <- substr(rownames(dds_results_TBX5_df), 2, 9)
dds_results_TBX5_df$mer8_3 <- substr(rownames(dds_results_TBX5_df), 3, 10)

# Merge
TEMP1 <- merge(dds_results_TBX5_df,
               TBX5_PBM_Escores, 
...
dds_results_TBX5_df_tiledEscores <- TEMP3

################################################################# Effect of flanking nucleotides (aggregate analysis)
correlation_test_results_df <- data.frame()
Unique_8mers <- unique(c(TBX5_PBM_Escores_PADIT$X8.mer_1_8[which(TBX5_PBM_Escores_PADIT$MPRAactivity == 1)], 
                         TBX5_PBM_Escores_PADIT$X8.mer.1_1_8[which(TBX5_PBM_Escores_PADIT$MPRAactivity == 1)]))

for(i in 1:length(Unique_8mers))
{
  dds_results_TBX5_df_tiledEscores_subset <- dds_results_TBX5_df_tiledEscores[which(dds_results_TBX5_df_tiledEscores$mer8_2 %in% Unique_8mers[i]),]
  if(nrow(dds_results_TBX5_df_tiledEscores_subset) < 8)
  {
  ...
  correlation_test_results <- cor.test(apply(dds_results_TBX5_df_tiledEscores_subset[, c("E.score_1", "E.score_3")], 1, mean), 
                                       dds_results_TBX5_df_tiledEscores_subset$log2FoldChange, 
                                       method = "pearson") 
  ...
  correlation_test_results_df[i, 3] <- nrow(dds_results_TBX5_df_tiledEscores_subset)
  correlation_test_results_df[i, 4] <- sum((dds_results_TBX5_df_tiledEscores_subset$MPRAactivity == 1)*1)
}
...
TBX5_PBM_Escores <- unique(TBX5_PBM_Escores)
...
hist(correlation_test_results_df$correlation_coeff, #[which(correlation_test_results_df$E.score > 0.4)], 
     ...
     xlim = c(-1, 1), 
     col = "dark green",
     xlab = "Pearson R (mean E-scores of flanking nucleotides VS TBX5 PADIT-seq log2FC)", 
     main = "Effect of flanking nucleotides on TBX5 PADIT-seq log2FC")

################################################################# Effect of flanking nucleotides (case by case basis)
kmers_to_highlight <- c("GAGGTGTA")
dds_results_TBX5_df_tiledEscores_subset2 <- dds_results_TBX5_df_tiledEscores[which(dds_results_TBX5_df_tiledEscores$mer8_2 == kmers_to_highlight),] 
dds_results_TBX5_df_tiledEscores_subset2$Colour <- "black"
dds_results_TBX5_df_tiledEscores_subset2$Colour[which(dds_results_TBX5_df_tiledEscores_subset2$MPRAactivity == 1)] <- "red"
plot(apply(dds_results_TBX5_df_tiledEscores_subset2[, c("E.score_1", "E.score_3")], 1, mean), 
     dds_results_TBX5_df_tiledEscores_subset2$log2FoldChange,
     cex = 3.0, pch = 19, 
     col = dds_results_TBX5_df_tiledEscores_subset2$Colour, 
     # xlim = c(0, 0.5), ylim = c(0, 3), 
     xlab = "Mean E-score", ylab = "PADIT-seq log2FC")
cor.test(apply(dds_results_TBX5_df_tiledEscores_subset2[, c("E.score_1", "E.score_3")], 1, mean), 
         dds_results_TBX5_df_tiledEscores_subset2$log2FoldChange)
...
Unique_8mers <- unique(c(TBX5_PBM_Escores_PADIT$X8.mer_1_8[which(TBX5_PBM_Escores_PADIT$MPRAactivity == 1)], 
                         TBX5_PBM_Escores_PADIT$X8.mer.1_1_8[which(TBX5_PBM_Escores_PADIT$MPRAactivity == 1)]))
dds_results_TBX5_df_tiledEscores_subset3 <- dds_results_TBX5_df_tiledEscores[which(dds_results_TBX5_df_tiledEscores$mer8_1 %in% Unique_8mers | 
                                                                                       dds_results_TBX5_df_tiledEscores$mer8_2 %in% Unique_8mers | 
                                                                                       dds_results_TBX5_df_tiledEscores$mer8_3 %in% Unique_8mers),] 

# lm
summary(lm(dds_results_TBX5_df_tiledEscores_subset3$log2FoldChange ~ 
             dds_results_TBX5_df_tiledEscores_subset3$E.score_1 + 
             dds_results_TBX5_df_tiledEscores_subset3$E.score_2 + 
             dds_results_TBX5_df_tiledEscores_subset3$E.score_3))
