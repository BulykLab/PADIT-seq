...
```{r}
# Read Peak File
ChIP_nexus_peaks_shuffled_random <- read.table(paste(paste(Working_dir, "Input_Files/ChIP-nexus_data", sep = "/"), "GSM6267788_sc_myc_cbf1_nexus_2_peaks_shufled1.bed.fa2.0.txt", sep = "/"), header = FALSE)
...
for(i in 1:nrow(ChIP_nexus_peaks_shuffled_random))
{
  ...
  CBF1_PBM_Escores_PADIT_df_subset <- CBF1_PBM_Escores_PADIT_df[which(CBF1_PBM_Escores_PADIT_df$X8.mer %in% PADIT_tiling$kmer),]
  CBF1_PBM_Escores_PADIT_df_subset <- CBF1_PBM_Escores_PADIT_df_subset[match(PADIT_tiling$kmer, CBF1_PBM_Escores_PADIT_df_subset$X8.mer),]
  PADIT_tiling <- CBF1_PBM_Escores_PADIT_df_subset
...
```{r}
# Read Peak File
ChIP_nexus_peaks <- read.table(paste(paste(Working_dir, "Input_Files/ChIP-nexus_data", sep = "/"), "GSM6267788_sc_myc_cbf1_nexus_2_peaks_seq.narrowPeak", sep = "/"), header = FALSE)
colnames(ChIP_nexus_peaks) <- c("chr", "start", "end", "PeakName", paste("V", 5:10, sep = ""), "DNA_sequence")

# Read Peak File
ChIP_nexus_peaks_shuffled_local <- read.table(paste(paste(Working_dir, "Input_Files/ChIP-nexus_data", sep = "/"), "seq_Cbf1.tsv", sep = "/"), header = FALSE)
...
# Write
write.table(ChIP_nexus_peaks, file = paste(paste(Working_dir, "Input_Files/ChIP-nexus_data", sep = "/"), "GSM6267788_sc_myc_cbf1_nexus_2_peaks_seq_numOverlappingBS.narrowPeak", sep = "/"), col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t")
...
process_peak <- function(peak_data, dds_results_CBF1_df, max_length) {
  ...
  dds_results_CBF1_df_subset <- dds_results_CBF1_df[dds_results_CBF1_df$TFBS %in% Sequence_10mers_DF$V1, ]
  dds_results_CBF1_df_subset <- dds_results_CBF1_df_subset[match(Sequence_10mers_DF$V1, dds_results_CBF1_df_subset$TFBS), ]
  
  Longest_1s_results <- Longest_1s(dds_results_CBF1_df_subset$MPRAactivity)
  
  if(Longest_1s_results$max_length == max_length && 
     Longest_1s_results$start_position > 4 && 
     (Longest_1s_results$start_position + max_length + 10) < nrow(dds_results_CBF1_df_subset)) {
    
    log2FC <- dds_results_CBF1_df_subset$log2FoldChange[(Longest_1s_results$start_position-4):(Longest_1s_results$start_position+max_length+3)]
...
```{r}
# format CBF1_PBM_Escores_PADIT_df
dds_results_CBF1_df <- CBF1_PBM_Escores_PADIT_df
colnames(dds_results_CBF1_df) <- c("TFBS", "E.score", "Z.score", "log2FoldChange", "padj", "MPRAactivity")
...
for(i in 1:nrow(ChIP_nexus_peaks)) {
  for(max_length in 3:5) {
    result <- process_peak(ChIP_nexus_peaks[i, ], dds_results_CBF1_df, max_length)
    if (!is.null(result)) {
...
write.table(DF3, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "CBF1_ChIP-nexus_3consecutive8mers_and4bpsFlank_log2FC.bed", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
write.table(DF4, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "CBF1_ChIP-nexus_4consecutive8mers_and4bpsFlank_log2FC.bed", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
write.table(DF5, file = paste(paste(Working_dir, "Output_Files", sep = "/"), "CBF1_ChIP-nexus_5consecutive8mers_and4bpsFlank_log2FC.bed", sep = "/"), col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
```
...
#
DF3 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "CBF1_ChIP-nexus_3consecutive8mers_and4bpsFlank_log2FC.bed", sep = "/"), header = FALSE)
Generate_PWM_seqlogo(substr(DF3$V4, 2, 17), 16)

#
DF4 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "CBF1_ChIP-nexus_4consecutive8mers_and4bpsFlank_log2FC.bed", sep = "/"), header = FALSE)
Generate_PWM_seqlogo(substr(DF4$V4, 2, 18), 17)

#
DF5 <- read.table(paste(paste(Working_dir, "Output_Files", sep = "/"), "CBF1_ChIP-nexus_5consecutive8mers_and4bpsFlank_log2FC.bed", sep = "/"), header = FALSE)
