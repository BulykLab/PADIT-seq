...
HOXD13_PBM_Escores_PADIT$padj <- apply(HOXD13_PBM_Escores_PADIT[,c("padj_1_8", "padj_2_9", "padj_3_10")], 1, median)
...
cor.test(HOXD13_PBM_Escores_PADIT_subset$E.score_1_8, HOXD13_PBM_Escores_PADIT_subset$Mean_log2FC)

# Plot (against z-scores)
HOXD13_PBM_Escores_PADIT_subset <- HOXD13_PBM_Escores_PADIT 
plot(HOXD13_PBM_Escores_PADIT_subset$Z.score_1_8[which(HOXD13_PBM_Escores_PADIT$Colour == "black")], 
     HOXD13_PBM_Escores_PADIT_subset$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT$Colour == "black")], 
     cex = 0.5, pch = 19, cex.axis = 2, cex.lab = 2,
     col = "black", 
     xlim = c(-2.5, 12.5), ylim = c(-0.3, 2.8), 
     xlab = "Z-score", ylab = "PADIT-seq log2 (ALFA-HOXD13 / No-DBD)")
points(HOXD13_PBM_Escores_PADIT_subset$Z.score_1_8[which(HOXD13_PBM_Escores_PADIT$Colour == "red")], 
       HOXD13_PBM_Escores_PADIT_subset$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT$Colour == "red")],
       cex = 0.5, pch = 19, col = "red")
abline(h = 0, col = "red")
cor.test(HOXD13_PBM_Escores_PADIT_subset$Z.score_1_8, HOXD13_PBM_Escores_PADIT_subset$Mean_log2FC)
...
# merge -2
TEMP1 <- merge(HOXD13_PBM_Escores_PADIT, HT_SELEX, by.x = "X8.mer_1_8", by.y = "Kmer", all.x = TRUE)
TEMP2 <- merge(TEMP1, HT_SELEX, by.x = "X8.mer.1_1_8", by.y = "Kmer",  all.x = TRUE)
TEMP2[is.na(TEMP2)] <- 0
...
HOXD13_PBM_Escores_PADIT_HTSELEX <- TEMP2[, which(!colnames(TEMP2) %in% c("Cycle1.x", "Cycle1.y", "Cycle2.x", "Cycle2.y", "Cycle3.x", "Cycle3.y", "Cycle4.x", "Cycle4.y"))]
...
# Merge
TEMP1 <- merge(HOXD13_PBM_Escores_PADIT_HTSELEX, ProBound_results, 
               by.x = "X8.mer_1_8", by.y = "trimmed_kmer")
TEMP2 <- merge(TEMP1, ProBound_results, 
               by.x = "X8.mer.1_1_8", by.y = "trimmed_kmer")
HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound <- TEMP2
HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound$log2_rel_affinity <- log2(apply(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound[, c("rel_affinity.x", "rel_affinity.y")], 1, max))
HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound <- HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound[, which(!colnames(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound) %in% c("rel_affinity.x", "rel_affinity.y"))]

################################################## Add FIMO data
# 1-8
All8mers_1 <- read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_HOXD13/FIMO", sep = "/"), "All_8mers.Match.MA0909.1.pos.1.8.txt", sep = "/"), header = FALSE)[, c(1, 8)]
colnames(All8mers_1) <- c("sequence_name", "p.value1")

# 2-9
All8mers_2 <- read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_HOXD13/FIMO", sep = "/"), "All_8mers.Match.MA0909.1.pos.2.9.txt", sep = "/"), header = FALSE)[, c(1, 8)]
colnames(All8mers_2) <- c("sequence_name", "p.value2")

# 3-10
All8mers_3 <- read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_HOXD13/FIMO", sep = "/"), "All_8mers.Match.MA0909.1.pos.3.10.txt", sep = "/"), header = FALSE)[, c(1, 8)]
colnames(All8mers_3) <- c("sequence_name", "p.value3")

### Merge-1
PWM_FIMO <- merge(merge(All8mers_1, All8mers_2, by = "sequence_name"), All8mers_3, by = "sequence_name")
PWM_FIMO$FIMO.p.value <- apply(PWM_FIMO[, paste("p.value", 1:3, sep = "")], 1, min)
PWM_FIMO <- PWM_FIMO[, c("sequence_name", "FIMO.p.value")]

### Merge-2
TEMP1 <- merge(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound, PWM_FIMO, 
               by.x = "X8.mer_1_8", by.y = "sequence_name")
TEMP2 <- merge(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound, PWM_FIMO, 
               by.x = "X8.mer.1_1_8", by.y = "sequence_name")
TEMP3 <- unique(rbind(TEMP1, TEMP2))
HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO <- TEMP3
HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$FIMO.p.value <- -log10(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$FIMO.p.value)
...
```{r}
library("DescTools")
Columns_To_Plot <- c("E.score_1_8", "Cycle1", "Cycle2", "Cycle3", "Cycle4", "log2_rel_affinity", "FIMO.p.value")
# Columns_To_Plot <- c("E.score_1_8", "Z.score_1_8")
for(k in 1:length(Columns_To_Plot))
{
  thresholds <- seq(min(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO[, Columns_To_Plot[k]]), 
                    max(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO[, Columns_To_Plot[k]]), length.out = 5000)
  fraction_red <- vector()
  fraction_black <- vector()
  for(i in 1:length(thresholds))
  {
    HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$AboveThreshold <- (HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO[, Columns_To_Plot[k]] > thresholds[i])*1
    fraction_red[i] <- nrow(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity == 1 & HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$AboveThreshold == 1),])
    fraction_black[i] <- nrow(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity != 1 & HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$AboveThreshold == 1),])
  }
  
  # print AUC
  # print AUC
  DF <- cbind(xvalues = fraction_black/(nrow(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO) - sum(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity)), 
              yvalues = fraction_red/sum(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity))
  DF <- as.data.frame(DF)
  ...
  {
    # Plot AUC curve
    plot(fraction_black/(nrow(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO) - sum(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity)), 
         fraction_red/sum(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity), 
         type = "l", col = "black")
  ...
  {
    # Plot AUC curve
    points(fraction_black/(nrow(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO) - sum(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity)), 
         fraction_red/sum(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity), 
         type = "l", col = "cyan3")
  }
  if(k == 6)
...
# Read HT-SELEX enriched 8-mers
table_1 <- read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_HOXD13", sep = "/"), "HOXD13_HT-SELEX_Cycle1_enriched_8mers_DefaultParameters.txt", sep = "/"), header = TRUE)
table_2 <- read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_HOXD13", sep = "/"), "HOXD13_HT-SELEX_Cycle2_enriched_8mers_DefaultParameters.txt", sep = "/"), header = TRUE)
table_3 <- read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_HOXD13", sep = "/"), "HOXD13_HT-SELEX_Cycle3_enriched_8mers_DefaultParameters.txt", sep = "/"), header = TRUE)
table_4 <- read.table(paste(paste(Working_dir, "Input_Files/HT-SELEX_data_HOXD13", sep = "/"), "HOXD13_HT-SELEX_Cycle4_enriched_8mers_DefaultParameters.txt", sep = "/"), header = TRUE)
...
```{r}
# Select Cycle
HOXD13_PBM_Escores_PADIT$Selected_Cycle <- HOXD13_PBM_Escores_PADIT$Cycle4

# Define Colours
HOXD13_PBM_Escores_PADIT$Colour <- "black"
HOXD13_PBM_Escores_PADIT$Colour[which(HOXD13_PBM_Escores_PADIT$MPRAactivity == 1)] <- "red"
HOXD13_PBM_Escores_PADIT$Colour[which(HOXD13_PBM_Escores_PADIT$Selected_Cycle == 1)] <- "cyan3"
HOXD13_PBM_Escores_PADIT_subset <- HOXD13_PBM_Escores_PADIT 
...
points(HOXD13_PBM_Escores_PADIT_subset$E.score_1_8[which(HOXD13_PBM_Escores_PADIT$Colour == "red")], 
       HOXD13_PBM_Escores_PADIT_subset$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT$Colour == "red")],
       cex = 0.5, pch = 19, col = "red")
points(HOXD13_PBM_Escores_PADIT_subset$E.score_1_8[which(HOXD13_PBM_Escores_PADIT$Colour == "cyan3")], 
       HOXD13_PBM_Escores_PADIT_subset$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT$Colour == "cyan3")],
...
HOXD13_PBM_Escores_PADIT_ProBound$Colour[which(HOXD13_PBM_Escores_PADIT_ProBound$MPRAactivity == 1 & HOXD13_PBM_Escores_PADIT_ProBound$log2_rel_affinity_sig == 1)] <- "cyan3"
plot(HOXD13_PBM_Escores_PADIT_ProBound$log2_rel_affinity[which(HOXD13_PBM_Escores_PADIT_ProBound$Colour == "black")],
     HOXD13_PBM_Escores_PADIT_ProBound$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT_ProBound$Colour == "black")], 
     pch = 19, col = HOXD13_PBM_Escores_PADIT_ProBound$Colour[which(HOXD13_PBM_Escores_PADIT_ProBound$Colour == "black")], xlim = c(-22, -1.5), ylim = c(-0.25, 2.75))
points(HOXD13_PBM_Escores_PADIT_ProBound$log2_rel_affinity[which(HOXD13_PBM_Escores_PADIT_ProBound$Colour != "black")],
     HOXD13_PBM_Escores_PADIT_ProBound$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT_ProBound$Colour != "black")], 
     pch = 19, col = HOXD13_PBM_Escores_PADIT_ProBound$Colour[which(HOXD13_PBM_Escores_PADIT_ProBound$Colour != "black")])
...
```{r}
# define colours
HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour <- "black"
HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity == 1)] <- "red"
HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$FIMO.p.value > -log10(0.001))] <- "cyan3"

# plot
plot(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$E.score_1_8[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour == "black")],
     HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour == "black")], 
     cex = 0.5, pch = 19, col = HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour == "black")], xlim = c(-0.5, 0.5), ylim = c(-0.25, 2.75))
points(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$E.score_1_8[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour != "black")],
     HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour != "black")], 
     cex = 0.5, pch = 19, col = HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour != "black")])
abline(h = 0, col = "red")

# boxplot
boxplot(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour == "red")],
        HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Mean_log2FC[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour == "cyan3")], 
        col = c("red", "cyan3"), notch = TRUE, outline = FALSE)
table(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour)
nrow(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO[which(HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$MPRAactivity == 1 & HOXD13_PBM_Escores_PADIT_HTSELEX_ProBound_FIMO$Colour == "cyan3"), ])
