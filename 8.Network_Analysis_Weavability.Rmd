...
######################################################### Write files for visualization with GEPHI
write.table(adj_matrix, paste(paste(Working_dir, "Output_Files", sep = "/"), "HOXD13_BindingSites_adj_matrix_gephi.csv", sep = "/"), row.names = TRUE, col.names = TRUE, quote = FALSE, sep = ";")
Node_attributes <- dds_results_HOXD13_df_sig[, c("TFBS", "log2FoldChange")]
colnames(Node_attributes) <- c("ID", "log2FoldChange")
write.table(Node_attributes, paste(paste(Working_dir, "Output_Files", sep = "/"), "HOXD13_node_attributes_for_gephi.csv", sep = "/"), row.names = FALSE, sep = ";")

```

# Network Analysis for TBX5
```{r}
# Read TBX5 PBM data
...
# 
dds_results_TBX5_df_sig <- unique(dds_results_TBX5_df[which(dds_results_TBX5_df$MPRAactivity == 1),])
dim(dds_results_TBX5_df_sig)

######################################################### Create Adjacency matrix
# Your vector of strings
strings_vector <- dds_results_TBX5_df_sig$TFBS
...
# Names
rownames(adj_matrix) <- dds_results_TBX5_df_sig$TFBS
colnames(adj_matrix) <- dds_results_TBX5_df_sig$TFBS
...
# 
columnNames <- colnames(dds_results_TBX5_df_sig)
TEMP <- merge(dds_results_TBX5_df_sig, comp$membership, by.x = "TFBS", by.y = 0)
dds_results_TBX5_df_sig <- TEMP
colnames(dds_results_TBX5_df_sig) <- c(columnNames, "Membership")

# 
dds_results_TBX5_df_sig$Membership <- (dds_results_TBX5_df_sig$Membership == 1)*1

# Number of nodes
nrow(dds_results_TBX5_df_sig)
...
# 
columnNames <- colnames(dds_results_TBX5_df_sig)
TEMP <- merge(dds_results_TBX5_df_sig, degree_vector, by.x = "TFBS", by.y = 0)
dds_results_TBX5_df_sig <- TEMP
colnames(dds_results_TBX5_df_sig) <- c(columnNames, "Degrees")

# 
# dds_results_TBX5_df_sig$Degrees[which(dds_results_TBX5_df_sig$Membership == 0)] <- 0

# Boxplot
boxplot(log2FoldChange ~ Degrees, data = dds_results_TBX5_df_sig, 
        notch = TRUE, outline = FALSE, col = "red", 
        xlab = "Number of edges", ylab = "PADIT-seq log2FC")
table(dds_results_TBX5_df_sig$Degrees)

# Pairwise wilcoxon test
pairwise.wilcox.test(dds_results_TBX5_df_sig$log2FoldChange, dds_results_TBX5_df_sig$Degrees)

# Correlation Test
cor.test(dds_results_TBX5_df_sig$log2FoldChange, dds_results_TBX5_df_sig$Degrees, 
         method = "pearson")

######################################################### Write files for visualization with GEPHI
write.table(adj_matrix, paste(paste(Working_dir, "Output_Files", sep = "/"), "TBX5_BindingSites_adj_matrix_gephi.csv", sep = "/"), row.names = TRUE, col.names = TRUE, quote = FALSE, sep = ";")
Node_attributes <- dds_results_TBX5_df_sig[, c("TFBS", "log2FoldChange")]
colnames(Node_attributes) <- c("ID", "log2FoldChange")
write.table(Node_attributes, paste(paste(Working_dir, "Output_Files", sep = "/"), "TBX5_node_attributes_for_gephi.csv", sep = "/"), row.names = FALSE, sep = ";")

```

# Network Analysis for NKX2-5
```{r}
...
# 
dds_results_NKX2.5_df_sig <- unique(dds_results_NKX2.5_df[which(dds_results_NKX2.5_df$MPRAactivity == 1),])
dim(dds_results_NKX2.5_df_sig)

######################################################### Create Adjacency matrix
# Your vector of strings
strings_vector <- dds_results_NKX2.5_df_sig$TFBS
...
# Names
rownames(adj_matrix) <- dds_results_NKX2.5_df_sig$TFBS
colnames(adj_matrix) <- dds_results_NKX2.5_df_sig$TFBS
...
# 
columnNames <- colnames(dds_results_NKX2.5_df_sig)
TEMP <- merge(dds_results_NKX2.5_df_sig, comp$membership, by.x = "TFBS", by.y = 0)
dds_results_NKX2.5_df_sig <- TEMP
colnames(dds_results_NKX2.5_df_sig) <- c(columnNames, "Membership")

# 
dds_results_NKX2.5_df_sig$Membership <- (dds_results_NKX2.5_df_sig$Membership == 1)*1

# Number of nodes
nrow(dds_results_NKX2.5_df_sig)
...
# 
columnNames <- colnames(dds_results_NKX2.5_df_sig)
TEMP <- merge(dds_results_NKX2.5_df_sig, degree_vector, by.x = "TFBS", by.y = 0)
dds_results_NKX2.5_df_sig <- TEMP
colnames(dds_results_NKX2.5_df_sig) <- c(columnNames, "Degrees")

# 
# dds_results_NKX2.5_df_sig$Degrees[which(dds_results_NKX2.5_df_sig$Membership == 0)] <- 0

# Boxplot
boxplot(log2FoldChange ~ Degrees, data = dds_results_NKX2.5_df_sig, 
        notch = TRUE, outline = FALSE, col = "red", 
        xlab = "Number of edges", ylab = "PADIT-seq log2FC")
table(dds_results_NKX2.5_df_sig$Degrees)

# Pairwise wilcoxon test
pairwise.wilcox.test(dds_results_NKX2.5_df_sig$log2FoldChange, dds_results_NKX2.5_df_sig$Degrees)

# Correlation test
cor.test(dds_results_NKX2.5_df_sig$log2FoldChange, dds_results_NKX2.5_df_sig$Degrees, 
         method = "pearson")

######################################################### Write files for visualization with GEPHI
write.table(adj_matrix, paste(paste(Working_dir, "Output_Files", sep = "/"), "NKX2.5_BindingSites_adj_matrix_gephi.csv", sep = "/"), row.names = TRUE, col.names = TRUE, quote = FALSE, sep = ";")
Node_attributes <- dds_results_NKX2.5_df_sig[, c("TFBS", "log2FoldChange")]
colnames(Node_attributes) <- c("ID", "log2FoldChange")
write.table(Node_attributes, paste(paste(Working_dir, "Output_Files", sep = "/"), "NKX2.5_node_attributes_for_gephi.csv", sep = "/"), row.names = FALSE, sep = ";")

```


# Network Analysis for PHO4
```{r}
# Read PHO4 PBM data
...
PHO4_PBM_Escores_PADIT$Median_baseMean <- ifelse(substr(PHO4_PBM_Escores_PADIT$X8.mer_1_8, 1, 5) == "ACGTG" | substr(PHO4_PBM_Escores_PADIT$X8.mer.1_1_8, 1, 5) == "ACGTG",
                                                 PHO4_PBM_Escores_PADIT$baseMean_2_9,
                                                 PHO4_PBM_Escores_PADIT$Median_baseMean)
PHO4_PBM_Escores_PADIT$Mean_log2FC <- ifelse(substr(PHO4_PBM_Escores_PADIT$X8.mer_1_8, 1, 5) == "ACGTG" | substr(PHO4_PBM_Escores_PADIT$X8.mer.1_1_8, 1, 5) == "ACGTG",
                                                 PHO4_PBM_Escores_PADIT$log2FoldChange_2_9,
                                                 PHO4_PBM_Escores_PADIT$Mean_log2FC)
PHO4_PBM_Escores_PADIT$padj <- ifelse(substr(PHO4_PBM_Escores_PADIT$X8.mer_1_8, 1, 5) == "ACGTG" | substr(PHO4_PBM_Escores_PADIT$X8.mer.1_1_8, 1, 5) == "ACGTG",
                                                 PHO4_PBM_Escores_PADIT$padj_2_9,
                                                 PHO4_PBM_Escores_PADIT$padj)
PHO4_PBM_Escores_PADIT$MPRAactivity <- ifelse(substr(PHO4_PBM_Escores_PADIT$X8.mer_1_8, 1, 5) == "ACGTG" | substr(PHO4_PBM_Escores_PADIT$X8.mer.1_1_8, 1, 5) == "ACGTG",
                                                 PHO4_PBM_Escores_PADIT$MPRAactivity_2_9,
                                                 PHO4_PBM_Escores_PADIT$MPRAactivity)

#
df1 <- PHO4_PBM_Escores_PADIT[,c("X8.mer_1_8", "Mean_log2FC", "MPRAactivity")]
colnames(df1) <- c("TFBS", "log2FoldChange", "MPRAactivity")
df2 <- PHO4_PBM_Escores_PADIT[,c("X8.mer.1_1_8", "Mean_log2FC", "MPRAactivity")]
colnames(df2) <- c("TFBS", "log2FoldChange", "MPRAactivity")
dds_results_PHO4_df <- rbind(df1, df2)

# 
dds_results_PHO4_df_sig <- unique(dds_results_PHO4_df[which(dds_results_PHO4_df$MPRAactivity == 1),])
dim(dds_results_PHO4_df_sig)

######################################################### Create Adjacency matrix
# Your vector of strings
strings_vector <- dds_results_PHO4_df_sig$TFBS
...
# Names
rownames(adj_matrix) <- dds_results_PHO4_df_sig$TFBS
colnames(adj_matrix) <- dds_results_PHO4_df_sig$TFBS
...
# 
columnNames <- colnames(dds_results_PHO4_df_sig)
TEMP <- merge(dds_results_PHO4_df_sig, comp$membership, by.x = "TFBS", by.y = 0)
dds_results_PHO4_df_sig <- TEMP
colnames(dds_results_PHO4_df_sig) <- c(columnNames, "Membership")

# 
dds_results_PHO4_df_sig$Membership <- (dds_results_PHO4_df_sig$Membership == 1)*1

# Number of nodes
nrow(dds_results_PHO4_df_sig)
...
# 
columnNames <- colnames(dds_results_PHO4_df_sig)
TEMP <- merge(dds_results_PHO4_df_sig, degree_vector, by.x = "TFBS", by.y = 0)
dds_results_PHO4_df_sig <- TEMP
colnames(dds_results_PHO4_df_sig) <- c(columnNames, "Degrees")

# 
# dds_results_PHO4_df_sig$Degrees[which(dds_results_PHO4_df_sig$Membership == 0)] <- 0

# Boxplot
boxplot(log2FoldChange ~ Degrees, data = dds_results_PHO4_df_sig, 
        notch = TRUE, outline = FALSE, col = "red", 
        xlab = "Number of edges", ylab = "PADIT-seq log2FC")
table(dds_results_PHO4_df_sig$Degrees)

# Pairwise wilcoxon test
pairwise.wilcox.test(dds_results_PHO4_df_sig$log2FoldChange, dds_results_PHO4_df_sig$Degrees)

######################################################### Write files for visualization with GEPHI
write.table(adj_matrix, paste(paste(Working_dir, "Output_Files", sep = "/"), "PHO4_BindingSites_adj_matrix_gephi.csv", sep = "/"), row.names = TRUE, col.names = TRUE, quote = FALSE, sep = ";")
Node_attributes <- dds_results_PHO4_df_sig[, c("TFBS", "log2FoldChange")]
colnames(Node_attributes) <- c("ID", "log2FoldChange")
write.table(Node_attributes, paste(paste(Working_dir, "Output_Files", sep = "/"), "PHO4_node_attributes_for_gephi.csv", sep = "/"), row.names = FALSE, sep = ";")

```

# Network Analysis for CBF1
```{r}
...
# Account for the effect of flanking nucleotides
CBF1_PBM_Escores_PADIT$Median_baseMean <- ifelse(substr(CBF1_PBM_Escores_PADIT$X8.mer_1_8, 1, 5) == "ACGTG" | substr(CBF1_PBM_Escores_PADIT$X8.mer.1_1_8, 1, 5) == "ACGTG",
                                                 CBF1_PBM_Escores_PADIT$baseMean_2_9,
                                                 CBF1_PBM_Escores_PADIT$Median_baseMean)
CBF1_PBM_Escores_PADIT$Mean_log2FC <- ifelse(substr(CBF1_PBM_Escores_PADIT$X8.mer_1_8, 1, 5) == "ACGTG" | substr(CBF1_PBM_Escores_PADIT$X8.mer.1_1_8, 1, 5) == "ACGTG",
                                                 CBF1_PBM_Escores_PADIT$log2FoldChange_2_9,
                                                 CBF1_PBM_Escores_PADIT$Mean_log2FC)
CBF1_PBM_Escores_PADIT$padj <- ifelse(substr(CBF1_PBM_Escores_PADIT$X8.mer_1_8, 1, 5) == "ACGTG" | substr(CBF1_PBM_Escores_PADIT$X8.mer.1_1_8, 1, 5) == "ACGTG",
                                                 CBF1_PBM_Escores_PADIT$padj_2_9,
                                                 CBF1_PBM_Escores_PADIT$padj)
CBF1_PBM_Escores_PADIT$MPRAactivity <- ifelse(substr(CBF1_PBM_Escores_PADIT$X8.mer_1_8, 1, 5) == "ACGTG" | substr(CBF1_PBM_Escores_PADIT$X8.mer.1_1_8, 1, 5) == "ACGTG",
                                                 CBF1_PBM_Escores_PADIT$MPRAactivity_2_9,
                                                 CBF1_PBM_Escores_PADIT$MPRAactivity)

#
df1 <- CBF1_PBM_Escores_PADIT[,c("X8.mer_1_8", "Mean_log2FC", "MPRAactivity")]
colnames(df1) <- c("TFBS", "log2FoldChange", "MPRAactivity")
df2 <- CBF1_PBM_Escores_PADIT[,c("X8.mer.1_1_8", "Mean_log2FC", "MPRAactivity")]
colnames(df2) <- c("TFBS", "log2FoldChange", "MPRAactivity")
dds_results_CBF1_df <- rbind(df1, df2)

# 
dds_results_CBF1_df_sig <- unique(dds_results_CBF1_df[which(dds_results_CBF1_df$MPRAactivity == 1),])
dim(dds_results_CBF1_df_sig)

######################################################### Create Adjacency matrix
# Your vector of strings
strings_vector <- dds_results_CBF1_df_sig$TFBS
...
# Names
rownames(adj_matrix) <- dds_results_CBF1_df_sig$TFBS
colnames(adj_matrix) <- dds_results_CBF1_df_sig$TFBS
...
# 
columnNames <- colnames(dds_results_CBF1_df_sig)
TEMP <- merge(dds_results_CBF1_df_sig, comp$membership, by.x = "TFBS", by.y = 0)
dds_results_CBF1_df_sig <- TEMP
colnames(dds_results_CBF1_df_sig) <- c(columnNames, "Membership")

# 
dds_results_CBF1_df_sig$Membership <- (dds_results_CBF1_df_sig$Membership == 1)*1

# Number of nodes
nrow(dds_results_CBF1_df_sig)
...
# 
columnNames <- colnames(dds_results_CBF1_df_sig)
TEMP <- merge(dds_results_CBF1_df_sig, degree_vector, by.x = "TFBS", by.y = 0)
dds_results_CBF1_df_sig <- TEMP
colnames(dds_results_CBF1_df_sig) <- c(columnNames, "Degrees")

# 
# dds_results_CBF1_df_sig$Degrees[which(dds_results_CBF1_df_sig$Membership == 0)] <- 0

# Boxplot
boxplot(log2FoldChange ~ Degrees, data = dds_results_CBF1_df_sig, 
        notch = TRUE, outline = FALSE, col = "red", 
        xlab = "Number of edges", ylab = "PADIT-seq log2FC")
table(dds_results_CBF1_df_sig$Degrees)

# Pairwise wilcoxon test
pairwise.wilcox.test(dds_results_CBF1_df_sig$log2FoldChange, dds_results_CBF1_df_sig$Degrees)

######################################################### Write files for visualization with GEPHI
write.table(adj_matrix, paste(paste(Working_dir, "Output_Files", sep = "/"), "CBF1_BindingSites_adj_matrix_gephi.csv", sep = "/"), row.names = TRUE, col.names = TRUE, quote = FALSE, sep = ";")
Node_attributes <- dds_results_CBF1_df_sig[, c("TFBS", "log2FoldChange")]
colnames(Node_attributes) <- c("ID", "log2FoldChange")
write.table(Node_attributes, paste(paste(Working_dir, "Output_Files", sep = "/"), "CBF1_node_attributes_for_gephi.csv", sep = "/"), row.names = FALSE, sep = ";")
...
# Correlation test
cor.test(dds_results_EGR1_df_sig$log2FoldChange, dds_results_EGR1_df_sig$Degrees, 
         method = "pearson")
