...
# Re-define NA
ZIF268_PBM_Escores_PADIT$padj_1_9[is.na(ZIF268_PBM_Escores_PADIT$padj_1_9)] <- 1
ZIF268_PBM_Escores_PADIT$padj_2_10[is.na(ZIF268_PBM_Escores_PADIT$padj_2_10)] <- 1

# Take median values
ZIF268_PBM_Escores_PADIT$Max_padj <- apply(ZIF268_PBM_Escores_PADIT[,c("padj_1_9", "padj_2_10")], 1, max)

# Redefine MPRAactivity
ZIF268_PBM_Escores_PADIT$MPRAactivity <- (ZIF268_PBM_Escores_PADIT$Mean_log2FC > 0 & ZIF268_PBM_Escores_PADIT$Max_padj < 0.10)*1 + (ZIF268_PBM_Escores_PADIT$Mean_log2FC > 0 & ZIF268_PBM_Escores_PADIT$Max_padj < 0.05)*1 + (ZIF268_PBM_Escores_PADIT$Mean_log2FC > 0 & ZIF268_PBM_Escores_PADIT$Max_padj < 0.01)*1

# Assign colour
ZIF268_PBM_Escores_PADIT$Colour <- "black"
ZIF268_PBM_Escores_PADIT$Colour[which(ZIF268_PBM_Escores_PADIT$MPRAactivity == 1)] <- "goldenrod"
ZIF268_PBM_Escores_PADIT$Colour[which(ZIF268_PBM_Escores_PADIT$MPRAactivity == 2)] <- "cyan3"
ZIF268_PBM_Escores_PADIT$Colour[which(ZIF268_PBM_Escores_PADIT$MPRAactivity == 3)] <- "red"
...
kmers_median_df <- kmers_median_df[, c("Alexa488_Chamber6_EGR1_800nM")]

```


# Put both orientations in 1 row
```{r}
...
{
  if(nchar(sequences_df$TFBS[i]) == 8)
  {
    sequences_df$TFBS[i] <- paste(sequences_df$TFBS[i], "G", sep = "")
  }
}

# Merge 9-mers
TEMP1 <- merge(ZIF268_PBM_Escores_PADIT, 
               sequences_df, 
               by.x = "X9.mer_1_9", by.y = "TFBS")
TEMP2 <- merge(ZIF268_PBM_Escores_PADIT, 
               sequences_df, 
               by.x = "X9.mer.1_1_9", by.y = "TFBS")
...
# Scatter plot
PADIT_PBM_DF <- PADIT_PBM_DF[which(PADIT_PBM_DF$Colour == "red" | PADIT_PBM_DF$E.score_1_9 < 0.40), ]
...
SNPs_DF <- read.table(paste(paste(Working_dir, "Input_Files/Custom_PBM/Design", sep = "/"), "SNPs_toTest_with_customPBM_EGR1.txt", sep = "/"), header = TRUE)
...
########################### 300nM
# Boxplot
boxplot(log10(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber8_EGR1_300nM)),
        log10(c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber8_EGR1_300nM)), 
        notch = FALSE, outline = FALSE)

# Add jittered points
points(jitter(rep(1, length(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber8_EGR1_300nM))), factor = 4),
         log10(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber8_EGR1_300nM)),
         pch = 19, col = alpha("red", 1), cex = 2)
points(jitter(rep(2, length(c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber8_EGR1_300nM))), factor = 4),
         log10(c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber8_EGR1_300nM)),
         pch = 19, col = alpha("red", 1), cex = 2)

# Test
wilcox.test(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber8_EGR1_300nM),
        c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber8_EGR1_300nM))
t.test(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber8_EGR1_300nM),
        c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber8_EGR1_300nM))

########################### 500nM
# Boxplot
boxplot(log10(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_REF$Alexa488_Chamber7_EGR1_500nM)),
        log10(c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_ALT$Alexa488_Chamber7_EGR1_500nM)), 
        notch = FALSE, outline = FALSE)

# Add jittered points
points(jitter(rep(1, length(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_REF$Alexa488_Chamber7_EGR1_500nM))), factor = 4),
         log10(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_REF$Alexa488_Chamber7_EGR1_500nM)),
         pch = 19, col = alpha("red", 1), cex = 2)
points(jitter(rep(2, length(c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_ALT$Alexa488_Chamber7_EGR1_500nM))), factor = 4),
         log10(c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_ALT$Alexa488_Chamber7_EGR1_500nM)),
         pch = 19, col = alpha("red", 1), cex = 2)

# Test
wilcox.test(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_REF$Alexa488_Chamber7_EGR1_500nM),
        c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_ALT$Alexa488_Chamber7_EGR1_500nM))
t.test(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_REF$Alexa488_Chamber7_EGR1_500nM),
        c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber5_EGR1_500nM, Alexa488_signal_df_SNP_ALT$Alexa488_Chamber7_EGR1_500nM))

########################### 800nM
# Boxplot
boxplot(log10(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber6_EGR1_800nM)),
        log10(c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber6_EGR1_800nM)), 
        notch = FALSE, outline = FALSE)

# Add jittered points
points(jitter(rep(1, length(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber6_EGR1_800nM))), factor = 4),
         log10(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber6_EGR1_800nM)),
         pch = 19, col = alpha("red", 1), cex = 2)
points(jitter(rep(2, length(c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber6_EGR1_800nM))), factor = 4),
         log10(c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber6_EGR1_800nM)),
         pch = 19, col = alpha("red", 1), cex = 2)

# Test
wilcox.test(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber6_EGR1_800nM),
        c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber6_EGR1_800nM))
t.test(c(Alexa488_signal_df_SNP_REF$Alexa488_Chamber6_EGR1_800nM),
        c(Alexa488_signal_df_SNP_ALT$Alexa488_Chamber6_EGR1_800nM))
...
# Redefine MPRAactivity
ZIF268_PBM_Escores_PADIT$MPRAactivity <- (ZIF268_PBM_Escores_PADIT$Mean_log2FC > 0 & ZIF268_PBM_Escores_PADIT$Max_padj < 0.05)*1
...
for(i in 1:nrow(SNPs_DF))
{
  ...
  dds_results_EGR1_df_subset_ALT <- dds_results_EGR1_df[which(dds_results_EGR1_df$TFBS %in% Sequence_9mers_DF$V1),]
  dds_results_EGR1_df_subset_ALT <- dds_results_EGR1_df_subset_ALT[match(Sequence_9mers_DF$V1, dds_results_EGR1_df_subset_ALT$TFBS),] 
  
  # concatenate columns
  dds_results_EGR1_df_subset <- cbind(dds_results_EGR1_df_subset_REF, dds_results_EGR1_df_subset_ALT)
  colnames(dds_results_EGR1_df_subset) <- c(paste(colnames(dds_results_EGR1_df_subset_REF), "REF", sep = "_"), 
                                            paste(colnames(dds_results_EGR1_df_subset_ALT), "ALT", sep = "_"))
  
  # PADIT-seq activity of the primary binding site
  dds_results_EGR1_df_subset_primary <-  dds_results_EGR1_df_subset[which( dds_results_EGR1_df_subset$TFBS_REF !=  dds_results_EGR1_df_subset$TFBS_ALT),]
  SNPs_DF$PrimaryBindingSite_activity[i] <- max(c(sum( dds_results_EGR1_df_subset_primary$log2FoldChange_REF[which( dds_results_EGR1_df_subset_primary$MPRAactivity_REF == 1)]),
                                                  sum(dds_results_EGR1_df_subset_primary$log2FoldChange_ALT[which(dds_results_EGR1_df_subset_primary$MPRAactivity_ALT == 1)])))
  
  # Remove rows containing the SNP
  dds_results_EGR1_df_subset <- dds_results_EGR1_df_subset[which(dds_results_EGR1_df_subset$TFBS_REF == dds_results_EGR1_df_subset$TFBS_ALT), ]
  
  # Store relevant information
  if(sum(dds_results_EGR1_df_subset$MPRAactivity_REF) == 0)
  {
  ...
  if(sum(dds_results_EGR1_df_subset$MPRAactivity_REF) > 0)
  {
    SNPs_DF$SecondaryBindingSite_num[i] <- sum(dds_results_EGR1_df_subset$MPRAactivity_REF)
    SNPs_DF$SecondaryBindingSite_activity[i] <- sum(dds_results_EGR1_df_subset$log2FoldChange_REF[which(dds_results_EGR1_df_subset$MPRAactivity_REF == 1)])
  }
}

# Is there a relationship between secondary binding sites and allelic difference? Yes! 
plot(abs(SNPs_DF$SecondaryBindingSite_activity), 
     abs(SNPs_DF$CustomPBM_Median_Diff))
cor.test(abs(SNPs_DF$SecondaryBindingSite_activity), 
         abs(SNPs_DF$CustomPBM_Median_Diff))

# Odds ratio analysis (MotifBreakR)
SNPs_DF$effect_formatted <- SNPs_DF$effect
SNPs_DF$effect_formatted[which(SNPs_DF$effect_formatted == "weak")] <- "strong"
fisher.test(table(SNPs_DF$effect_formatted, SNPs_DF$CustomPBM_pvalue_adj < 0.05))

# Odds ratio analysis (PADIT-seq)
fisher.test(table(SNPs_DF$PADIT_prediction, SNPs_DF$CustomPBM_pvalue_adj < 0.05))

# 
TEMP <- SNPs_DF[which(SNPs_DF$SecondaryBindingSite_activity == 0),]

# SNPs_DF[which(SNPs_DF$SecondaryBindingSite_activity - SNPs_DF$PrimaryBindingSite_activity > abs(SNPs_DF$Diff_log2FC)), ]
...
# Add small random noise to zeros
set.seed(123456)  # for reproducibility
alleleEffectSize_mod <- SNPs_DF$alleleEffectSize*-1
zero_indices <- which(alleleEffectSize_mod == 0)
alleleEffectSize_mod[zero_indices] <- rnorm(length(zero_indices), 
                                          mean=0, 
                                          sd=min(abs(SNPs_DF$alleleEffectSize[SNPs_DF$alleleEffectSize != 0]))/100)
