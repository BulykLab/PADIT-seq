...
for(Select_cycle in c("input", "cycle1", "cycle2", "cycle3", "cycle4"))
{
  ...
  #
  if(nrow(EGR1_CHIP_tmp) > 20000)
  {
    EGR1_CHIP <- as.data.frame(EGR1_CHIP_tmp[sample(1:nrow(EGR1_CHIP_tmp), 20000), ])
    colnames(EGR1_CHIP) <- c("V1")
  }
  else
  {
    EGR1_CHIP <- as.data.frame(EGR1_CHIP_tmp)
    colnames(EGR1_CHIP) <- c("V1")
  }
...
for(i in 1:nrow(EGR1_PBM_All60mers))
{
  ...
  EGR1_PBM_All60mers$max_log2FC_active_only[i] <- max(2^dds_results_EGR1_df_subset$log2FoldChange)
  if(sum(dds_results_EGR1_df_subset$MPRAactivity) == 0)
  {
    EGR1_PBM_All60mers$sum_log2FC_active_only[i] <- 0
    EGR1_PBM_All60mers$max_log2FC_active_only[i] <- 0
    next
  }
...
EGR1_PBM_All60mers_subset <- EGR1_PBM_All60mers_subset[which(EGR1_PBM_All60mers_subset$num_active > 0), ]

# 1
summary(lm(log10(EGR1_PBM_All60mers_subset$Alexa488Adjusted) ~ EGR1_PBM_All60mers_subset$sum_log2FC_active_only + EGR1_PBM_All60mers_subset$max_log2FC_active_only))
summary(lm(log10(EGR1_PBM_All60mers_subset$Alexa488Adjusted) ~ EGR1_PBM_All60mers_subset$num_active + EGR1_PBM_All60mers_subset$max_log2FC_active_only))

# relative importance
library(relaimpo)
model <- lm(log10(Alexa488Adjusted) ~ max_log2FC_active_only + num_active, data = EGR1_PBM_All60mers_subset)
model <- lm(log10(Alexa488Adjusted) ~ max_log2FC_active_only + sum_log2FC_active_only, data = EGR1_PBM_All60mers_subset)
